...
class Agent:
    ...
    @unwrap_json_response
    @fail_if_not_ok("Failed to register service")
    def register_service(self, connection_id, schema):
        """Set connection metadata."""
        return post(
            self.url,
            f"/connections/{connection_id}/register-service",
            json={"schema": schema},
        )

    @unwrap_json_response
    @fail_if_not_ok("Failed to query services")
    def query_services(self, connection_id, schema):
        """Set connection metadata."""
        return post(
            self.url,
            f"/connections/{connection_id}/query-services",
            json={"schema": schema},
        )
